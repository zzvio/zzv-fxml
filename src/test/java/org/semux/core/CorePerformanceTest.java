...
public class CorePerformanceTest {
    ...
    private static final Logger logger = Logger.getLogger(CorePerformanceTest.class.getName());
    ...
    public void testSortDelegate() {
        ...
        logger.info(String.format("Perf_delegate_sort: %s μs", (t2 - t1) / 1_000));
    }
    ...
    public void testTransactionProcessing() {
        ...
        long t2 = System.nanoTime();
        logger.info(String.format("Perf_transaction_1: %s μs/tx", (t2 - t1) / 1_000 / repeat));
        ...
        exec.execute(txs, chain.getAccountState().track(), chain.getDelegateState().track(),
                ...
                0);
        t2 = System.nanoTime();
        logger.info(String.format("Perf_transaction_2: %s μs/tx", (t2 - t1) / 1_000 / repeat));
    }
}
