...
public class BlockchainImpl implements Blockchain {
    ...
    private static final Logger logger = Logger.getLogger(BlockchainImpl.class.getName());
    ...
    public synchronized void addBlock(Block block) {
        ...
        if (number != genesis.getNumber() && number != latestBlock.getNumber() + 1) {
            logger.severe(String.format("Adding wrong block: number = %s, expected = %s", number, latestBlock.getNumber() + 1));
    ...
    protected boolean validateBlock(Block block, AccountState asTrack, DelegateState dsTrack, boolean validateVotes) {
        try {
            ...
            if (!block.validateHeader(header, latest.getHeader())) {
                logger.severe("Invalid block header");
                return false;
            }
            ...
            if (config.checkpoints().containsKey(header.getNumber()) &&
                ...
                logger.severe(String.format("Checkpoint validation failed, checkpoint is %s => %s, getting %s", header.getNumber(),
            ...
            if (!block.validateTransactions(header, transactions, config.network())) {
                logger.severe("Invalid transactions");
                return false;
            }
            if (transactions.stream().anyMatch(tx -> this.hasTransaction(tx.getHash()))) {
                logger.severe("Duplicated transaction hash is not allowed");
                return false;
            }
            ...
            logger.severe(String.format("Unexpected exception during block validation", e));
            return false;
        }
    }
    ...
    public boolean validateBlockVotes(Block block) {
        ...
        if (block.getVotes().stream().anyMatch(sig -> !validators.contains(Hex.encode(sig.getAddress())))) {
            logger.warning("Block votes are invalid");
            return false;
        }
        ...
        if (block.getVotes().stream()
            ...
            logger.warning(String.format("Not enough votes, required (2/3+) = %s, actual = %s", twoThirds, block.getVotes().size()));
            return false;
        }
        ...
        try {
            ...
            for (long i = 1; i <= target; i++) {
                ...
                if (i % 1000 == 0) {
                    ...
                    logger.info(String.format("Loaded %s / %s blocks", i, target));
                }
                imported++;
            }
            ...
            logger.info(String.format("Database upgraded: found blocks = %s, imported = %s, took = %s", latestBlockNumber, imported,
                    TimeUtil.formatDuration(Duration.between(begin, end))));
        } catch (IOException e) {
            logger.severe(String.format("Failed to upgrade database", e));
        }
    }
