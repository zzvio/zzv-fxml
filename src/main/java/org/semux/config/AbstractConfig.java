...
import static org.semux.Network.*;
...
public abstract class AbstractConfig implements Config, ChainSpec {
  ...
  private static final long[][][] periods = new long[3][64][];
  ...
  protected Set<MessageCode> netPrioritizedMessages =
      new HashSet<>(
          Arrays.asList(
  ...
  protected List<String> netDnsSeedsMainNet =
      Collections.unmodifiableList(Arrays.asList("mainnet.semux.org", "mainnet.semux.net"));
  ...
  protected String[] apiPublicServices = {"blockchain", "account", "delegate", "tool"};
  protected String[] apiPrivateServices = {"node", "wallet"};
  // =========================
  // BFT consensus
  // =========================
  ...
  @Override
  public String getPrimaryValidator(
      List<String> validators, long height, int view, boolean uniformDist) {
    if (uniformDist) {
  ...
  private int getUniformDistPrimaryValidatorNumber(int size, long height, long view) {
    ...
    do {
      BigInteger seed =
          BigIntegerUtil.random(BigInteger.valueOf(height))
              .xor(BigIntegerUtil.random(BigInteger.valueOf(view)))
              .add(subView);
      deterministicRand = BigIntegerUtil.random(seed).mod(BigInteger.valueOf(size)).intValue();
  ...
  public File chainDir(Network network) {
    return new File(
        rootDir, Constants.CHAIN_DIR + File.separator + network.name().toLowerCase(Locale.ROOT));
  }
    ...
    try (FileInputStream in = new FileInputStream(f)) {
      ...
      for (Object k : props.keySet()) {
        ...
        switch (name) {
          ...
          case "api.public":
            apiPublicServices =
                Stream.of(props.getProperty(name).trim().split(","))
                    .map(String::trim)
                    .toArray(String[]::new);
            break;
          case "api.private":
          ...
          case "log":
            {
              Level level = Level.parse(props.getProperty(name).trim());
              Logger rootLogger = LogManager.getLogManager().getLogger("");
              rootLogger.setLevel(level);
              for (Handler h : rootLogger.getHandlers()) {
                h.setLevel(level);
              }
              Logger.getLogger(getClass().getName()).info("CMD LogLevel = " + rootLogger.getLevel().toString());
              break;
            }
          default:
            logger.severe(
                String.format("Unsupported option: %s = %s", name, props.getProperty(name)));
            break;
        }
      }
    } catch (Exception e) {
      logger.severe(String.format("Failed to load config file: %s", f, e));
    }
  }
    ...
    if (apiEnabled) {
      if ("YOUR_API_USERNAME".equals(apiUsername) || "YOUR_API_PASSWORD".equals(apiPassword)) {
        throw new ConfigException(
            "Please change your API username/password from the default values.");
      }

      if (Arrays.stream(apiPublicServices)
          .anyMatch(x -> Arrays.stream(apiPrivateServices).anyMatch(y -> y.equals(x)))) {
        throw new ConfigException(
            "There are services which belong to both api.public and api.private");
      }
    }
  }
}
