...
public class SemuxBft implements BftManager {
    private static final Logger logger = Logger.getLogger(SemuxBft.class.getName());
    ...
    protected void eventLoop() {
        while (!Thread.currentThread().isInterrupted() && status != Status.STOPPED) {
            try {
                ...
                logger.warning(String.format("Unexpected exception in event loop", e));
            }
        }
    }
    ...
    public void stop() {
        if (status != Status.STOPPED) {
            ...
            if (!events.offer(ev)) {
                logger.severe(String.format("Failed to add an event to message queue: ev = %s", ev));
            }
        }
    }
    ...
    protected void enterNewHeight() {
        ...
        logger.info(String.format("Entered new_height: height = %s, # validators = %s", height, validators.size()));
        if (isValidator()) {
            if (this.config.network() == Network.MAINNET && !SystemUtil.bench()) {
                logger.severe("You need to upgrade your computer to join the BFT consensus!");
    ...
    protected void enterPropose() {
        ...
        logger.info(String.format("Entered propose: height = %s, view = %s, primary = %s, # connected validators = 1 + %s", height,
        ...
        if (isPrimary()) {
            ...
            logger.finest(String.format("Proposing: %s", proposal));
    ...
    protected void enterValidate() {
        ...
        logger.info(String.format("Entered validate: proposal = %s, votes = %s %s %s", proposal != null, validateVotes,
    ...
    protected void enterPreCommit() {
        ...
        logger.info(String.format("Entered pre_commit: proposal = %s, votes = %s %s %s", proposal != null, validateVotes,
                precommitVotes, commitVotes));
    ...
    protected void enterCommit() {
        ...
        logger.info(String.format("Entered commit: proposal = %s, votes = %s %s %s", proposal != null, validateVotes, precommitVotes,
                commitVotes));
    ...
    protected void enterFinalize() {
        ...
        logger.info(String.format("Entered finalize: proposal = %s, votes = %s %s %s", proposal != null, validateVotes,
                precommitVotes, commitVotes));
    ...
    protected void onNewView(Proof p) {
        ...
        if (p.getHeight() == height // at same height
            ...
            // switch view
            logger.finest(String.format("Switching view because of NEW_VIEW message: %s", p.getView()));
            jumpToView(p.getView(), p, null);
        }
    }

    protected void onProposal(Proposal p) {
        logger.finest(String.format("On proposal: %s", p));
        ...
        if (p.getHeight() == height // at the same height
            ...
            logger.finest(String.format("Proposal accepted: height = %s, view = %s", p.getHeight(), p.getView()));
            ...
            if (view == p.getView()) {
                ...
                // switch view
                logger.finest("Switching view because of PROPOSE message");
    ...
    protected void onVote(Vote v) {
        logger.finest(String.format("On vote: %s", v));
        ...
        case BFT_PROPOSAL: {
            ...
            if (p.getHeight() == height) {
                if (p.validate()) {
                    ...
                    logger.finest(String.format("Invalid proposal from %s", channel.getRemotePeer().getPeerId()));
        ...
        case BFT_VOTE: {
            ...
            if (vote.getHeight() == height) {
                if (vote.revalidate()) {
                    ...
                    logger.finest(String.format("Invalid vote from %s", channel.getRemotePeer().getPeerId()));
                    channel.getMessageQueue().disconnect(ReasonCode.BAD_PEER);
                }
            }
            break;
    ...
    protected Block proposeBlock() {
        ...
        logger.finest(String.format("Block creation: # txs = %s, time = %s ms", includedTxs.size(), t2 - t1));
    ...
    protected boolean validateBlockProposal(BlockHeader header, List<Transaction> transactions) {
        try {
            ...
            if (!block.validateHeader(header, latest.getHeader())) {
                logger.warning("Invalid block header");
                return false;
            }
            ...
            if (header.getTimestamp() - TimeUtil.currentTimeMillis() > config.bftMaxBlockTimeDrift()) {
                logger.warning("A block in the future is not allowed");
                return false;
            }
            if (!Arrays.equals(header.getCoinbase(), proposal.getSignature().getAddress())) {
                logger.warning("The coinbase should always equal to the proposer's address");
                return false;
            }
            ...
            if (transactions.stream().anyMatch(tx -> chain.hasTransaction(tx.getHash()))) {
                logger.warning("Duplicated transaction hash is not allowed");
                return false;
            }
            ...
            if (!block.validateResults(header, results)) {
                logger.severe("Invalid transaction results");
                return false;
            }
            block.setResults(results); // overwrite the results

            long t2 = TimeUtil.currentTimeMillis();
            logger.finest(String.format("Block validation: # txs = %s, time = %s ms", transactions.size(), t2 - t1));

            validBlocks.put(ByteArray.of(block.getHash()), block);
            return true;
        } catch (Exception e) {
            logger.severe(String.format("Unexpected exception during block proposal validation", e));
            return false;
        }
    }
        ...
        public synchronized void stop() {
            if (t != null) {
                try {
                    ...
                    logger.warning("Failed to stop consensus timer");
                    Thread.currentThread().interrupt();
                }
                t = null;
            }
    ...
    public class Broadcaster implements Runnable {
        ...
        public synchronized void stop() {
            if (t != null) {
                try {
                    ...
                    logger.severe("Failed to stop consensus broadcaster");
                    Thread.currentThread().interrupt();
                }
                t = null;
            }
        ...
        public void broadcast(Message msg) {
            if (!queue.offer(msg)) {
                logger.severe(String.format("Failed to add a message to the broadcast queue: msg = %s", msg));
            }
        }
    }
