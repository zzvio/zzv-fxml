...
public class SystemUtil {
  ...
  public static String getIp() {
    try {
      ...
      logger.warning("Failed to retrieve your public IP address");

      try {
        return InetAddress.getLocalHost().getHostAddress();
      } catch (Exception e2) {
        logger.warning("Failed to retrieve your localhost IP address");
      }
    }
  ...
  public static boolean bench() {
    ...
    if (getNumberOfProcessors() < 2) {
      logger.info(String.format("# of CPU cores = %s", getNumberOfProcessors()));
      return false;
    }

    // check memory
    if (getTotalMemorySize() < 3L * 1024L * 1024L * 1024L) {
      logger.info(String.format("Total physical memory size = %s MB", getTotalMemorySize() / 1024 / 1024));
      return false;
    }
   ...
   * Change localization.
   *
   * @param locale the target localization.
   */
  public static void setLocale(Locale locale) {
    try {
      ...
      logger.severe(String.format("Unable to change localization.", e));
    }
  }
  ...
  public static Object getImplementationVersion() {
    if (version == null) {
      try {
        version =
            IOUtil.readStreamAsString(
                    SemuxCli.class.getClassLoader().getResourceAsStream("VERSION"))
                .trim();
  ...
  /**
   * Returns whether Microsoft Visual C++ 2012 Redistributable Package is installed.
   *
   * @return
   */
  public static boolean isWindowsVCRedist2012Installed() {
    ...
    try {
      ...
      logger.severe(String.format("Failed to read windows registry", e));
      return false;
    }
  }

  /**
   * Determine if the current Java runtime supports the Java Platform Module System. Credits to:
