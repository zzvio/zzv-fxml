...
public class HdKeyGenerator {
    ...
    private static final Logger logger = Logger.getLogger(HdKeyGenerator.class.getName());
    ...
    public HdKeyPair getChildKeyPair(HdKeyPair parent, long child, boolean isHardened) {
        ...
        case BIP32_ED25519:
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine(String.format("parent, kLP = " + Hex.encode(kLP)));
                logger.fine(String.format("parent, kRP = " + Hex.encode(kRP)));
                logger.fine(String.format("parent,  AP = " + Hex.encode(AP)));
                logger.fine(String.format("parent,  cP = " + Hex.encode(cP)));
            }
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine(String.format("child, IL = " + Hex.encode(IL)));
                logger.fine(String.format("child, IR = " + Hex.encode(IR)));
                logger.fine(String.format("child,  A = " + Hex.encode(A)));
                logger.fine(String.format("child,  c = " + Hex.encode(c)));
            }
