...
public class Program {
    ...
    private ProgramResult createContractImpl(DataWord value, byte[] programCode, byte[] newAddress, long gas) {
        ...
        if (logger.isLoggable(Level.FINEST)) {
            logger.finest(String.format("CREATE: %s", internalTx));
        }
        ...
        if (result.getException() == null && !result.isRevert()) {
            ...
            if (logger.isLoggable(Level.FINEST)) {
                logger.finest(String.format("Contract run halted by Exception: contract: [%s], exception: [%s]",
                        HexUtil.toHexString(newAddress),
                        result.getException()));
            }
    ...
    private ProgramResult callContractImpl(MessageCall msg, PrecompiledContract contract) {
        ...
        if (logger.isLoggable(Level.FINEST)) {
            logger.finest(String.format("CALL: %s", internalTx));
        }
    ...
    public void spendGas(long gasValue, String cause) {
        logger.finest(String.format("Spend: cause = [%s], gas = [%s]", cause, gasValue));
    ...
    public void refundGas(long gasValue, String cause) {
        logger.finest(String.format("Refund: cause = [%s], gas = [%s]", cause, gasValue));

        getResult().refundGas(gasValue);
    }

    public void futureRefundGas(long gasValue) {
        logger.finest(String.format("Future refund added: [%s]", gasValue));
