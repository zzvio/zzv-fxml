...
public class TransactionExecutor {
    ...
    protected boolean prepare() {
        ...
        if (txGas.add(BigInteger.valueOf(gasUsedInTheBlock)).compareTo(blockGasLimit) > 0) {
            logger.warning("Too much gas used in this block");
            return false;
        }

        if (txGas.compareTo(BigInteger.valueOf(basicTxCost)) < 0) {
            logger.warning(String.format("Not enough gas to cover basic transaction cost: required = %s, actual = %s", basicTxCost,
                    txGas));
            return false;
        }
        ...
        if (reqNonce != txNonce) {
            logger.warning(String.format("Invalid nonce: required = %s, actual = %s", reqNonce, txNonce));
            return false;
        }
        ...
        if (!isCovers(senderBalance, totalCost)) {
            logger.warning(String.format("Not enough balance: required = %s, actual = %s", totalCost, senderBalance));
            return false;
        }
